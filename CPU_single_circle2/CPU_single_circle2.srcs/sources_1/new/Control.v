...
assign ALUsrc=~(opcode==R_format_op||opcode==BEQ_op);
assign RegWrite=(opcode==R_format_op)||opcode==LW_op||opcode==ADDI_op;
assign ALUOp[0]= (opcode==SW_op) || (opcode==R_format_op)||(opcode==ADDI_op);
assign ALUOp[1]= (opcode==BEQ_op) || (opcode==R_format_op)||(opcode==ADDI_op);
assign Jump= opcode==J_op;
endmodule
