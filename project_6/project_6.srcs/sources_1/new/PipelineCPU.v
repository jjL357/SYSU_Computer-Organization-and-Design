...
module PipelineCPU(clk,q,select,t1,temp,memwbMemtoReg);
input clk;
output [15:0] q;
output [1:0] select;
output [31:0] t1,temp;
...
wire [31:0] pcout,nopcount,t3,t4,readData_1,readData_2;
wire nop;
assign select = ForwardA;
assign temp = idexreadData1in ;
assign t1=ForwardC;
...
PC upc(clk,pcin,pc,nop,pcout,jumpAddr,branchAddr,Jump,(op==6'b000100&&Zero||op==6'b000101&&!Zero) && (nopcount != 2) && (idexMemRead == 0 || (ifidRs != writeReg && ifidRt != writeReg) ) ,t3,t4);
...
MUX #(32)mux99(readData1,writeData,memwbRegWrite&&memwbwriteReg==ifidRs&&(op==6'b000100||op==6'b000101),readData_1);
MUX #(32)mux100(readData2,writeData,memwbRegWrite&&memwbwriteReg==ifidRt&&(op==6'b000100||op==6'b000101),readData_2);
MUX3to1 #(32) mux10(readData_1,ALUresult,exmemALUresult,ForwardC,idexreadData1in);
MUX3to1 #(32) mux11(readData_2,ALUresult,exmemALUresult,ForwardD,idexreadData2in);
