...
module ForwardUnit(exmemRegWrite, memwbRegWrite,exmemwriteReg, memwbwriteReg,idexRs,idexRt, ForwardA,ForwardB);
    ...
    initial begin 
        ...
        if(exmemRegWrite && exmemwriteReg != 0) begin
         ...
         else if(memwbRegWrite && memwbwriteReg != 0) begin
...
always@(*) begin
       if(memwbRegWrite && memwbwriteReg != 0 ||exmemRegWrite && exmemwriteReg != 0)begin
            if(memwbRegWrite && memwbwriteReg != 0) begin
                if(memwbwriteReg == idexRs)  ForwardA = 2'b01;
                    else ForwardA = 2'b00; 
                if(memwbwriteReg == idexRt)  ForwardB = 2'b01;
                    else ForwardB = 2'b00; 
            end 
            if(exmemRegWrite && exmemwriteReg != 0) begin
                 if(exmemwriteReg == idexRs)  ForwardA = 2'b10; 
