...
module PipelineCPU(clk,q,temp,select,ALUa,ALUb,memwbMemtoReg);
input clk;
output [15:0] q;
output [1:0] select;
output [31:0] ALUa,ALUb,temp;
...
assign select = ForwardD;
assign temp=exmemwriteReg;
...
IFIDRegister ifid(clk,pc,pcin,ins,ifidop,ifidpcOut,ifidpcinOut,ifidinsOut,ifidRs,ifidRt,ifidRd );

ForwardUnit2 forward2(clk,BEQ||BNE,idexRegWrite,exmemRegWrite,ifidRs,ifidRt ,writeReg,exmemwriteReg, ForwardC,ForwardD);
Control control(ifidop, funct, RegDst, Jump, BEQ, BNE, MemRead, MemtoReg,ALUOp, MemWrite, ALUSrc, RegWrite, ExtSel);
