...
wire [31:0]ins_1,pc_2,pc_3,pcout;
wire [4:0]w1,w2,w3,wr1,wr2,wr3;
wire [31:0]IDEX_Reg1,IDEX_Reg2;
wire [31:0]result_1,result_2,b_data1,b_data2,baddresst,at,bt,pcj,pcb;
...
assign ifbranch=zero3 & (ifid_ins[31:26]==6'b000100||ifid_ins[31:26]==6'b000101);
...
assign zero2=~zero;
assign IDEX_Reg1=IDEX_ins[25:21];
assign IDEX_Reg2=IDEX_ins[20:16];
assign aa=ra;
assign bb=MEMWB_writereg;
assign result2=(jump1||ins_1[31:26]==6'b000000&&ins_1[5:0]==6'b001000)?32'b0:result;
HazardDetectUnit hdu1(clk,ifid_pc,ifid_ins,ifid_rs,ifid_rt,wr1,IDEX_MemRead,pcout,lw_zero,ccc);
Adder#(32) add1({{14{ifid_ins[15]}},ifid_ins[15:0],2'b00},ifid_pcIn,b_address);
//Mulx#(32)m1(b_address,pcIn,ifbranch,pc1);
//Mulx#(32)m2(j_address,pc1,Jump||(opcode==6'b000000&&funct==6'b001000),pc2);
Mulx#(32)bnop(b_address,pcIn,ifbranch,pcb);
Mulx#(32)jnop(j_address,pcIn,Jump||(opcode==6'b000000&&funct==6'b001000),pcj);
PC p1(clk,pcIn,pc,lw_zero,pcout,pcj,pcb,Jump||(opcode==6'b000000&&funct==6'b001000),ifbranch&&ccc!=2&&(IDEX_MemRead!=1||(ifid_rs!=wr1&&ifid_rt!=wr1)));
PCAdder4 p2(pc,pcIn);
IFID_Registers ifid(clk&lw_zero,pcout,lw_zero,pc,pcIn,ins,ifid_pc,ifid_pcIn,ifid_ins,ifid_rs,ifid_rt,ifid_rd);
InsMem uInsMem(pc,ins);
Control c1(lw_zero,opcode,RegDst,Jump,Branch,MemRead,MemtoReg,ALUOp,MemWrite,ALUsrc,RegWrite);
ALUControl a1(IDEX_ins[5:0],IDEX_ALUOp,ALU_control);
Mulx#(5)m3(Reg3,Reg2,RegDst,Write_Reg);////0选rt写入，1选rd写入
Registers r1(ifid_rs,ifid_rt,clk,MEMWB_regwrite,MEMWB_writereg,MEMWB_result,reading_data1,reading_data2,ifid_rd,MEMWB_RegDst,opcode,ra,ifid_pcIn);
IDEX_Registers idex(clk,lw_zero,pcout,MemRead,ifid_pc,ifid_ins,ALUOp,RegDst,ALUsrc,RegWrite,MemtoReg,MemWrite,ifid_ins[15:0],Jump,Write_Reg,ins_1,IDEX_pc,reading_data1,reading_data2,IDEX_MemRead,IDEX_RegDst,IDEX_ALUsrc,IDEX_RegWrite,IDEX_MemtoReg,IDEX_MemWrite,IDEX_imm,IDEX_data1,IDEX_data2,IDEX_ins,IDEX_ALUOp,jump1,wr1);
Mulx#(32)m4({{16{IDEX_imm[15]}},IDEX_imm},result_2,IDEX_ALUsrc,b);//ALUsrc0选寄存器，1选立即数或是地址
ForwardingUnit fu(IDEX_Reg1,IDEX_Reg2,IDEX_data1,IDEX_data2,EXMEM_regwrite,wr2,EXMEM_result,MEMWB_regwrite,MEMWB_writereg,MEMWB_result,result_1,result_2,ifid_ins[25:21],ifid_ins[20:16],reading_data1,reading_data2,IDEX_RegWrite,wr1,result,zero,at,bt);
...
EXMEM_Registers ex1(clk,result3,result_2,IDEX_pc,result2,IDEX_RegDst,IDEX_RegWrite,IDEX_MemWrite,wr1,IDEX_MemtoReg,EXMEM_pc,EXMEM_result,EXMEM_RegDst,EXMEM_regwrite,EXMEM_MemWrite,wr2,EXMEM_MemtoReg,aaa,IDEX_ins,EXMEM_ins);
...
Mulx#(32)m6(32'b0,Write_data,EXMEM_ins==0,Write_data2);
MEMWB_Registers mem1(clk,EXMEM_pc,Write_data2,EXMEM_RegDst,EXMEM_regwrite,wr2,MEMWB_pc,MEMWB_result,MEMWB_RegDst,MEMWB_regwrite,MEMWB_writereg);
endmodule
